...
# Periodic re-resolve can hit the same NXDOMAIN every interval; do not warn each tick.
_DNS_KEEP_PREVIOUS_LOG_INTERVAL_SEC = 60.0
_last_dns_keep_previous_log_at = 0.0
_DNS_RERESOLVE_FAIL_LOG_INTERVAL_SEC = 60.0
_last_dns_reresolve_fail_log_at = 0.0
...
def log_dns_keep_previous_throttled(raw, fingerprint) -> None:
    """Warn when a re-resolve keeps the live dial plan; rate-limit sustained NXDOMAIN."""
    global _last_dns_keep_previous_log_at
    now = time.monotonic()
    if now - _last_dns_keep_previous_log_at < _DNS_KEEP_PREVIOUS_LOG_INTERVAL_SEC:
        return
    _last_dns_keep_previous_log_at = now
    logger.warning(
...
def log_dns_reresolve_failure_throttled(message: str) -> None:
    """Rate-limit unexpected periodic DNS rebuild failures (with stack)."""
    global _last_dns_reresolve_fail_log_at
    now = time.monotonic()
    if now - _last_dns_reresolve_fail_log_at < _DNS_RERESOLVE_FAIL_LOG_INTERVAL_SEC:
        return
    _last_dns_reresolve_fail_log_at = now
...
def _lookup_hostname(host: str, port: int, *, log_failures: bool = True) -> List[BackendAddress]:
    """
    ...
    When ``log_failures`` is false (periodic re-resolve with a live dial plan),
    skip per-host error logs; the caller emits a throttled keep-previous warning.
    """
    import threading
    from concurrent.futures import Future, TimeoutError as FuturesTimeout

    def _fail(message: str, *args) -> List[BackendAddress]:
        if log_failures:
    ...
    *,
    ...
    for orig in addresses:
        if orig.kind == AddressKind.HOSTNAME:
            # Re-resolve (require_all): suppress per-host errors; caller throttles
            # a single keep-previous warning instead of logging every check tick.
            candidates = _lookup_hostname(
                orig.host, orig.port, log_failures=not require_all_hostnames,
            )
            if not candidates:
    ...
    *,
    ...
    try:
        ...
        if previous is not None:
            log_dns_keep_previous_throttled(raw, previous[2])
            return previous
        if addresses:
