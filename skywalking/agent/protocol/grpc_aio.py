    ...
    async def watch_dns_reresolve(self):
        ...
        while not self._closed:
            try:
                ...
                logger.exception('Periodic aio collector DNS re-resolve failed')
        ...
        if self.state in (None, grpc.ChannelConnectivity.IDLE):
            ...
            try:
                ...
                if peeked is not None:
                    # Scope the nudge to the channel generation we peeked so a
                    # concurrent DNS rebuild cannot apply a stale state.
