...
class SamplingServiceBase:
    ...
    def register_cds_watcher(self, *, replace: bool = False) -> None:
        """Register CDS watcher; ``replace`` rebinds after fork/force re-init."""
        if self._rate_watcher is not None and not replace:
            return
...
class SamplingServiceAsync(SamplingServiceBase):
    ...
    def __init__(self):
        # threading.Lock: try_sampling/force_sampled are sync and may run off the
        # event loop (request threads) while CDS mutates rate on the dispatch task.
        self.lock = Lock()
        SamplingServiceBase.__init__(self)
