...
| agent_force_tls | SW_AGENT_FORCE_TLS | <class 'bool'> | False | Use TLS for gRPC/HTTP with the OAP. One-way TLS uses the process trust store unless agent_ssl_trusted_ca_path points to a CA file. For gRPC, the server certificate CN/SAN must match the host of the first usable backend in agent_collector_backend_services (port stripped via grpc.ssl_target_name_override; :authority stays host:port). |
| agent_ssl_trusted_ca_path | SW_AGENT_SSL_TRUSTED_CA_PATH | <class 'str'> |  | PEM of the CA that signed the OAP server certificate. If this file exists, the agent uses TLS even when agent_force_tls is false (Java agent.ssl_trusted_ca_path). Absolute path or relative to the process working directory; symlinks to regular files are followed. Empty means no custom CA (Java defaults to /ca/ca.crt inside the agent package; Python has no package-root CA). PEM files larger than 256KiB are rejected. |
| agent_ssl_cert_chain_path | SW_AGENT_SSL_CERT_CHAIN_PATH | <class 'str'> |  | PEM of the agent certificate chain for mTLS. Used only when the CA file exists and agent_ssl_key_path is also set (Java agent.ssl_cert_chain_path). Missing files log a warning and keep one-way TLS. Symlinks to regular files are followed; 256KiB size cap. |
| agent_ssl_key_path | SW_AGENT_SSL_KEY_PATH | <class 'str'> |  | PEM of the agent private key for mTLS (unencrypted). PKCS#1 is converted to PKCS#8 like Java PrivateKeyUtil. Used with agent_ssl_cert_chain_path (Java agent.ssl_key_path). Symlinks to regular files are followed; 256KiB size cap. |
