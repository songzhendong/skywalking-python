...
Pass a comma-separated list. The agent opens **one** gRPC channel and lets C-core `pick_first` fail over (same idea as Node `sw-static`). Each process shuffles the preferred backend at channel build; `:authority` and TLS peer-name checks use the **first configured** endpoint via `grpc.default_authority` (not the shuffled channel target).
