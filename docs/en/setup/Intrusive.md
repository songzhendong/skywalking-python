...
- Channel `:authority` uses `grpc.default_authority` = the first configured endpoint (before shuffle). With TLS, BoringSSL hostname checks use that endpoint's **host** (port stripped via `grpc.ssl_target_name_override`), so every backend cert CN/SAN must cover that host (e.g. `oap` for `oap:11800`).

#### gRPC / HTTP TLS and mTLS

Aligned with Java `TLSChannelBuilder` (Kafka protocol does not use these options):
...
- mTLS is on only when the CA file exists **and** both `agent_ssl_cert_chain_path` and `agent_ssl_key_path` are readable PEMs. Missing cert/key logs a warning and stays one-way TLS (Java parity; does not abort start). With a CA file present, `agent_force_tls` is optional.
- Paths are absolute or relative to the process working directory (symlinks to regular files are followed, e.g. Kubernetes secret mounts). Each PEM is capped at 256KiB. PKCS#1 (`BEGIN RSA PRIVATE KEY`) is converted to PKCS#8 like Java `PrivateKeyUtil`; passphrase-encrypted private keys are not supported.
- Real-OAP e2e: `tests/e2e/case/grpc/ssl/` and `tests/e2e/case/grpc/mtls/` (Java `simple/ssl` / `simple/mtls` parity).
